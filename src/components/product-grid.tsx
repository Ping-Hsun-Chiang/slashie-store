...
export default function ProductGrid({ products }: { products: Product[] }) {
  if (products.length === 0) {
    return (
      <p className="text-zinc-500 dark:text-zinc-400">目前尚無上架商品。</p>
    );
  }
    ...
    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {products.map((product) => (
        <Link
          ...
          className="group overflow-hidden rounded-2xl border border-black/[.08] bg-white transition-shadow hover:shadow-md dark:border-white/[.145] dark:bg-zinc-950"
        >
